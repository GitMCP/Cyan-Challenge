...
class FarmController {
	async index(req, res) {
		const results = await Farm.findAll({where: { deleted_at: null},attributes: ['id', 'code', 'name', 'created_at'],order: [['id', 'ASC']], include:  [{model: Harvest, as: 'ownerHarvest', attributes: ['id', 'code', 'start_date', 'end_date']}, {model: User, as: 'author', attributes: ['id', 'name', 'email']}]});
	...
	async create(req, res) {
		...
		const codeExists = await Farm.findOne({ where: { code: req.body.code }});
		if (codeExists) {
			return res.status(400).json({ error: 'There\'s already a farm with that code!' });
		}
		...
		return res.json(await Farm.findByPk(id, {attributes: ['id', 'code', 'name'], include:{model: Harvest, as: 'ownerHarvest', attributes: ['id', 'code', 'start_date', 'end_date']}}));
	}
	...
	async delete(req, res) {
		...
		if(req.userId != farm.author_id){
			return res.status(400).json({ error: 'Sorry, you are not this Farm\'s author' });
		}
